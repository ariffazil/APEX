...
- DOMAIN_ROLE: ASI Deliberative Relay — cross-model deliberation, bridge, resilience contract. A2A agent on port 3002. Telegram interface.
...
- Container: `hermes-agent:v1.0.4` running healthy.
...
- ACPX bridge: Hermes spawns as child agent via stdio JSON-RPC (`hermes-acp-sops`).
...
- HERMES is the ASI Deliberative Relay — bridges multiple AI models for constitutional deliberation.
...
- Known gap: Hermes→GEOX/WEALTH use mismatched auth (ED25519 vs JWT). Transport matrix needed.


## 3. THE 999 SEAL (SESSION LOG)

- 2026-05-15 | Omega | ARIF.md created. SOT baseline established. HERMES previously had zero SOT documentation.
- 2026-05-14 | Omega | Dead Telegram subsystem removed (~290 lines). ACPX bridge validated (Hermes + Kimi Code as child agents).
...
- ENTRYPOINTS:
  - `npm start` → Start HERMES gateway
  - `hermes-acp-sops --accept-hooks` → ACPX child agent via SOPS
...
- HARD_BLOCK: None. Container healthy. Telegram connected.
...
|---------|--------|---------|
| `npm start` | ✅ | HERMES gateway on 3002 |
...
- [Q]: Add upstream remote for HERMES?
- [CONTEXT]: Sovereign decided HERMES stays in AAA repo. No separate remote needed. Ω₀ = 0.0 (sovereign verdict).
- [Q]: Fix HERMES→GEOX auth mismatch (ED25519 vs JWT)?
...
- [ ] Document HERMES→GEOX transport matrix (auth format per MCP endpoint)
- [ ] Add A2A routing skill for Hermes→GEOX via AAA-A2A
