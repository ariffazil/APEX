...
- **Constitution Hash Propagation**: Verify every A2A handshake against the federation's canonical constitution hash.
- **Deterministic Deliberation**: Run multi-turn reasoning under arifOS floor constraints — no consciousness claims, no hallucination, no self-authorization.
- **A2A Mesh Bridge**: Relay messages between federation agents while preserving governance context.
...
It does **not** judge (888) or seal (999). Those remain in arifOS.
...
| Field | Value |
|-------|-------|
| Role | **ASI Deliberative Relay** |
| Governing kernel | `arifOS MIND (Ω)` |
| Constitution hash | Verified on every A2A handshake |
| Fallback chain | SEA-LION → Ollama → deterministic rule |
| Runtime | `src/server.js` (Express, CommonJS) |
| Port | `127.0.0.1:3002` |
...
| Organ | Human Meaning | System Role |
|---|---|---|
| **ARIF / APEX** | Final human authority | F13 sovereign veto |
| **AAA** | Operator cockpit | Identity, A2A gateway, session control |
| **A-FORGE** | Execution shell | Runs tools, executes approved actions |
| **arifOS** | Governance kernel | Checks evidence, risk, authority, verdicts |
